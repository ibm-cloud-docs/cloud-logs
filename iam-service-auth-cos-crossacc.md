...
You must configure the S2S authorization in the account where the {{site.data.keyword.cos_full_notm}} bucket is available.
{: important}
...
- The target service, {{site.data.keyword.cos_full_notm}} account defines where you must create the authorization between the {{site.data.keyword.logs_full_notm}} instance and the {{site.data.keyword.cos_full_notm}} bucket.
...
- If you create an authorization between a service in another account and a target service in your current account, you need to have access only to the target resource. That is, When you configure the authorization, you must get the account ID of the the {{site.data.keyword.logs_full_notm}} instance. 


## Service access roles
{: #iam-service-auth-cos-roles-crossacc}

You must grant `Writer` role to {{site.data.keyword.logs_full_notm}} to work with {{site.data.keyword.cos_full_notm}} buckets.
...
3. Select **Other account** as the source account. Then, enter the account ID of the source account.
...
4. Select `Cloud Logs` as the source service. Then, set the scope of the access.
    ...
    Select **Specific resources** to grant access to a single {{site.data.keyword.logs_full_notm}} instance. Enter the instance ID in the *Service Instance* field.
...
```sh
ibmcloud iam authorization-policy-create logs cloud-object-storage "Writer" [--source-service-instance-name SOURCE_SERVICE_INSTANCE_NAME | --source-service-instance-id SOURCE_SERVICE_INSTANCE_ID] [--source-service-account ACCOUNT_GUID] [--target-service-instance-name TARGET_SERVICE_INSTANCE_NAME | --target-service-instance-id TARGET_SERVICE_INSTANCE_ID] [--target-resource-type RESOURCE_TYPE] [--target-resource RESOURCE]
```
{: codeblock}

ibmcloud iam authorization-policy-create { SOURCE_SERVICE_NAME TARGET_SERVICE_NAME ROLE_NAME1,ROLE_NAME2... [--source-service-instance-name SOURCE_SERVICE_INSTANCE_NAME | --source-service-instance-id SOURCE_SERVICE_INSTANCE_ID] [--source-service-account ACCOUNT_GUID]

Where you can set the following parameters to grant access to a single bucket:
- `ACCOUNT_GUID`: Enter the account ID where the {{site.data.keyword.logs_full_notm}} instance is available. An account ID is a 32 character, unique account identifier.
- `SOURCE_SERVICE_INSTANCE_NAME`: Enter the {{site.data.keyword.logs_full_notm}} instance name.
- `TARGET_SERVICE_INSTANCE_NAME`: Enter the  {{site.data.keyword.cos_full_notm}} instance name.

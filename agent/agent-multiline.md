...
## The {{site.data.keyword.agent}} default multiline configuration
...
## Changing the default multiline parser
{: #agent-multiline-parser-change}

Fluent bit supports different multiline parsers for `docker`, `cri` `java`, `go`, and `python`. For more information, see [Built-in multiline parsers](https://docs.fluentbit.io/manual/administration/configuring-fluent-bit/multiline-parsing#built-in-multiline-parsers){: external}.
...
## Modifying the {{site.data.keyword.agent}} configuration with the Multiline parser

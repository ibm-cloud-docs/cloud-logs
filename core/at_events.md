...
The following table lists the events that are generated by {{site.data.keyword.logs_full_notm}} for enrichments:
...
{: #at_events_audit_events_metrics}

The following tables lists the audit events metrics that are generated by {{site.data.keyword.logs_full_notm}}:

| Event                                               | Description                |
|---------------------------------------               |----------------------------|
| `logs.events-2-metrics.update`                      |  E2M definition changes       |
| `logs.metrics-data-ingress.send`                    |  Sending metrics data   |
| `logs.metrics-recording-rule.read`                  |	Viewing metrics recording rules |
| `logs.metrics-recording-rule.manage`                |	Managing metrics recording rules|
| `logs.metrics-tco-policy.read`                      |	Viewing existing TCO policies including listing blocked metrics|
| `logs.metrics-tco-policy.manage`                    |	Viewing and modifying existing metrics TCO policies, and creating new ones, including blocking and unblocking metrics|
| `logs.metrics-data-api-high.read`                   |	Querying metrics in the high tier (frequent search)|
| `logs.metrics-data-api-low.read`                    |	Querying metrics in the low tier (archive)|
| `logs.metrics-analytics-high.read`                  |	Reading mapping statistics (High) |
| `logs.metrics-analytics-low.read`                   |	Reading mapping statistics (low) |
| `logs.data-usage.read`                              |	Viewing data usage metrics |
| `logs.data-usage.manage`                            |	Managing data usage metrics |
| `logs.metrics-alert.read`                           |	Reading alert definitions |
| `logs.metrics-alert.manage`                         |	Defining and modifying meteics alert settings |
| `logs.metrics-priority.update`                      |	Defining priority changes |
{: caption="Events for metrics permissions mapping" caption-side="top"}


## Audit event catalog
{: #at_events_audit-event_catalog}

The following tables lists the audit event catalog that are generated by {{site.data.keyword.logs_full_notm}}:

| Event                                            | Description                |
|---------------------------------------------------|----------------------------|
| `logs.events-2-metrics.create`                   |  Creating events metrics  |
| `logs.events-2-metrics.update`                   |  Updating events metrics |
| `logs.events-2-metrics.delete`                   |  Deleting events metrics |
| `logs.events-2-metrics.get`                      |  Getting events metrics |
| `logs.events-2-metrics.list`.                    |  Listing events metrics |
| `logs.recording-rule.create`                     |  Creating recording rules |
| `logs.recording-rule.update`                     |  Updating recording rules |
| `logs.recording-rule.delete`                     |  Deleting recording rules |
| `logs.recording-rule-group.create`               |  Creating recording rules group |
| `logs.recording-rule-group.update`               |  Updating recording rules group |
| `logs.recording-rule-group.delete`               |  Deleting recording rules group |
| `logs.recording-rule-group-set.create`           |  Creating recording rules group set |
| `logs.recording-rule-group-set.update`           |  Updating recording rules group set |
| `logs.recording-rule-group-set.delete`           |  Deleting recording rules group set |
| `logs.metrics.data-api.read`                     |  Reading metrics data api |
| `logs.metrics.priority.update`                   |  Updating metrics priority |
{: caption="Events for audit event catalog" caption-side="top"}

...
- In {{site.data.keyword.cos_full}}, you can configure object lifecycle policies to automatically delete files from your buckets. For more information, see [Deleting stale data with expiration rules](/docs/cloud-object-storage?topic=cloud-object-storage-expiry).
    ...
    You can use tags as a filter option that allows expiration rules to apply to objects that contain a matching tag. The tag filter is provided as a container that specifies a key string and value string. The key string must be less than 128 characters. For {{site.data.keyword.cos_full}}, the key must be `ICL_ARCHIVE_RETENTION`. The value string must be less than 256 characters.

    If no prefix, tag or object size is configured, the policy will apply to all objects in the bucket.

    In {{site.data.keyword.logs_full_notm}}, you can use tags attached to TCO policies to help you manage automatically the log data in a bucket by configuring object lifecycle policies that limit the scope based on tags. For more information, see [Retention tags](/docs/cloud-logs?topic=cloud-logs-retention-tags).
    ...
    To use different lifecycle periods for logs data ingested through different data pipelines, you must configure retention tags in {{site.data.keyword.logs_full_notm}} and lifecycle policies filtering by tag appropriately.
    {: tip}

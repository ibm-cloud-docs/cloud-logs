...
* `Timestamp` or `Severity` are a log record metadata fields. In Lucene queries, these fields need to be prefixed with `coralogix.`. In DataPrime queries, these fields need to be  prefixed with `$m.`.
...
If you don't restrict queries in {{site.data.keyword.frequent-search}} to a searching a field, the search will be made on all indexed log record fields:

* Lucene queries: Don't prefix a query term with <field> when searching on indexed log record fields. This includes log record label and metadata fields.
...
You can use field-based filters and queries in {{site.data.keyword.frequent-search}} to search indexed log record fields:
...
* Queries: Lucene and DataPrime queries can be used to search for only specified indexed fields.
   ...
   * DataPrime: Use field accessors `$d.<data field>`, `$l.<label field>`, or `$m.<metadata field>` to query the specified field.
...
Each {{site.data.keyword.logs_full_notm}} service instance has a separate set of {{site.data.keyword.frequent-search}} indices. The first index in the set is created when the service instance is created. Once a day, a new index is created and added to the set. At the same time, the oldest index is removed if it has reached its expiration date.
...
* Use [TCO policies](/docs/cloud-logs?topic=cloud-logs-tco-optimizer) or [Block parsing rules](/docs/cloud-logs?topic=cloud-logs-parse-block-rule) to prevent that log records are stored.
